  ...
  return (
    <div className="relative group no-print">
      ...
      <Input
        ...
        className="pl-12 pr-4 h-14 bg-slate-900/40 border border-white/5 rounded-2xl focus:border-blue-500/30 focus:ring-4 focus:ring-blue-500/10 focus:shadow-[0_0_20px_rgba(59,130,246,0.15)] transition-all duration-300 font-medium text-white text-sm md:text-lg placeholder-slate-500"
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
      />
      
      {searchTerm && (
        <Card className="absolute top-full left-0 right-0 mt-3 z-50 bg-slate-900/95 backdrop-blur-2xl border border-white/10 shadow-2xl rounded-2xl overflow-hidden max-h-64 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2 duration-200">
          <CardContent className="p-0">
            {filteredProducts.length === 0 ? (
              ...
              filteredProducts.map(product => (
                ...
                >
                  <div>
                    <p className="font-bold text-white group-hover/item:text-blue-400 transition-colors uppercase">{product.name}</p>
                  ...
                  <Package className="w-4 h-4 text-slate-500 group-hover/item:text-blue-400 group-hover/item:scale-110 transition-all duration-200" />
                </button>
              ))
            )}
          </CardContent>
