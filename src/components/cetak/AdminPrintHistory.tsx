...
export function AdminPrintHistory() {
  ...
  // No auto-expansion needed as dates are opened in modal dialog
          ...
          {filteredHistory.length > itemsPerPage && (
            <div className="flex items-center justify-between px-6 py-4.5 border-t border-white/5 bg-slate-950/20">
              <span className="text-xs text-slate-500 font-semibold">
                Halaman {page} dari {totalPages} ({filteredHistory.length}{" "}
                riwayat)
              </span>
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={page === 1}
                  className="bg-slate-900/40 border border-white/5 hover:border-blue-500/30 text-white hover:bg-white/5 h-9 w-9 p-0 rounded-xl transition-all duration-200"
                >
