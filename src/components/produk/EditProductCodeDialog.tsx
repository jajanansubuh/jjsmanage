...
export function EditProductCodeDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="bg-zinc-950 border-zinc-800 text-white backdrop-blur-3xl shadow-2xl rounded-[2.5rem] p-0 max-w-md overflow-hidden gap-0">
        <div className="p-8 pb-4">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 ml-1">Suplier *</label>
              <Popover open={isSupplierPopoverOpen} onOpenChange={setIsSupplierPopoverOpen}>
                <PopoverTrigger
                  className="w-full h-12 px-4 bg-zinc-900 border border-zinc-800 rounded-2xl text-white text-sm font-bold flex items-center justify-between hover:bg-white/5 transition-all cursor-pointer outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50"
                >
                  <span className="truncate">
                    {suppliers.find(s => s.id === form.supplierId)?.name || "-- Pilih Suplier --"}
                  </span>
                  <ChevronDown className={cn("w-4 h-4 text-zinc-500 transition-transform duration-200", isSupplierPopoverOpen && "rotate-180")} />
                </PopoverTrigger>
                <PopoverContent
                  className="p-0 bg-zinc-900 border-zinc-800 text-white w-[var(--radix-popover-trigger-width)] z-100 shadow-2xl rounded-2xl overflow-hidden"
                  align="start"
                  sideOffset={8}
                >
                  <div className="p-3 border-b border-zinc-800 bg-zinc-950">
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="h-10 pl-9 bg-zinc-900 border-zinc-800 rounded-xl text-white placeholder:text-zinc-600 focus:ring-blue-500/20 focus:border-blue-500/50"
