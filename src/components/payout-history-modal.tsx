        ...
        <div className="p-6 md:p-8 border-b border-white/5 bg-white/[0.02]">
          <DialogHeader>
            <DialogTitle className="text-xl md:text-2xl font-black text-white flex items-center gap-3">
              ...
              Riwayat Transaksi
              {supplierName && <span className="text-amber-400/50 ml-1 hidden sm:inline">— {supplierName}</span>}
            </DialogTitle>
            {supplierName && <p className="text-amber-400/50 text-xs font-bold sm:hidden mt-1">{supplierName}</p>}
            <DialogDescription className="text-slate-400 text-sm font-medium mt-2">
              Seluruh transaksi consignment yang diinput oleh Admin.
            </DialogDescription>
          </DialogHeader>
          ...
          {!loading && transactions.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 md:gap-3 mt-5">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-xl border border-white/5">
                ...
                <span className="text-[10px] md:text-xs font-bold text-slate-300">{transactions.length} Trx</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-xl border border-white/5">
                <TrendingUp className="w-3.5 h-3.5 text-blue-400" />
                <span className="text-[10px] md:text-xs font-bold text-slate-300">{formatCurrency(totalRevenue)}</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
                <History className="w-3.5 h-3.5 text-emerald-400" />
                <span className="text-[10px] md:text-xs font-bold text-emerald-400">{formatCurrency(totalProfit80)}</span>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="md:hidden p-4 space-y-3">
            {loading ? (
              <div className="py-20 text-center space-y-3">
                ...
                <span className="text-slate-500 text-sm font-medium">Memuat data...</span>
              </div>
            ) : transactions.length === 0 ? (
              <div className="py-20 text-center text-slate-500 italic text-sm">Belum ada riwayat transaksi.</div>
            ) : (
              transactions.map((t) => {
                ...
                return (
                  <div key={t.id} className={cn(
                    "bg-white/5 rounded-2xl border border-white/5 p-4 transition-all",
                    isExpanded && "ring-1 ring-amber-500/30 bg-white/10"
                  )}>
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-right">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 block">Mitra Jjs</span>
                        <p className="text-base font-black text-emerald-400">+{formatCurrency(t.profit80)}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 pt-3 border-t border-white/5">
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Nota</span>
                        <p className="text-xs font-mono font-bold text-slate-400">{t.noteNumber || "—"}</p>
                      </div>
                      <div className="text-right">
                         <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Omzet</span>
                         <p className="text-xs font-bold text-slate-400">{formatCurrency(t.revenue)}</p>
                      </div>
                    </div>
                    {showExpand && (
                      <button 
                        onClick={() => toggleRow(t.id)}
                        className="w-full mt-3 pt-3 border-t border-white/5 flex items-center justify-center gap-1 text-[10px] font-black uppercase tracking-widest text-amber-400"
                      >
                    ...
                    {isExpanded && (
                       <div className="mt-3 pt-3 border-t border-white/5 space-y-3 animate-in slide-in-from-top-2 duration-300">
                          <div className="flex items-center gap-1.5">
                            <Scissors className="w-3 h-3 text-slate-500" />
                            <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">Potongan</span>
                          </div>
                          ...
                          {t.notes && <p className="text-[10px] text-slate-500 italic px-1">📝 {t.notes}</p>}
                       </div>
                    )}
                  </div>
                )
