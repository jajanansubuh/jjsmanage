...
export default function DepositsPage() {
  ...
  const handlePrint = () => {
    ...
    const totalPayout = filteredAndSortedData.reduce((sum, item) => sum + item.dailyProfit, 0);

    const tableRows = filteredAndSortedData.map((item, i) => `
      <tr>
        ...
        <td style="padding:10px;border-bottom:1px solid #eee">${item.ownerName || '-'}</td>
        <td style="padding:10px;border-bottom:1px solid #eee;font-weight:bold">${item.bankName || '-'}</td>
        <td style="padding:10px;border-bottom:1px solid #eee;font-family:monospace">${item.accountNumber || '-'}</td>
        <td style="padding:10px;border-bottom:1px solid #eee;text-align:right;font-weight:bold">Rp ${new Intl.NumberFormat('id-ID').format(item.dailyProfit)}</td>
      </tr>
    `).join('');
      ...
      <html>
        <head>
          ...
          <style>
            ...
            th { background: #f5f5f5; padding: 12px 10px; text-align: left; font-size: 11px; text-transform: uppercase; border-bottom: 2px solid #ddd; }
            .total { margin-top: 30px; text-align: right; font-size: 18px; font-weight: bold; border-top: 2px solid #333; padding-top: 15px; }
        ...
        <body>
          ...
          <div class="total">Total Seluruhnya: Rp ${new Intl.NumberFormat('id-ID').format(totalPayout)}</div>
